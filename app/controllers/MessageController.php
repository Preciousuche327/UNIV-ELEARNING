...
class MessageController {
    ...
    public function chat() {
        ...
        if ($userType === 'Student') {
            ...
            $stmt = $this->pdo->prepare("SELECT UserID, Username, Email FROM users WHERE UserType = 'Instructor' AND Status = 'Approved' ORDER BY Username ASC");
            $stmt->execute();
            $contacts = $stmt->fetchAll();
        } else if ($userType === 'Instructor') {
            // Instructors can message any student
            $stmt = $this->pdo->prepare("SELECT UserID, Username, Email FROM users WHERE UserType = 'Student' ORDER BY Username ASC");
            $stmt->execute();
            $contacts = $stmt->fetchAll();
        } else {
        ...
        if ($activeContactId) {
            $stmt = $this->pdo->prepare("SELECT UserID, Username, Email, UserType FROM users WHERE UserID = ?");
